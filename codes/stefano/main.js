...
function renderScene() {
  ...
  for ( let i = 0; i < game.objects.plane.geometry.attributes.distortion.count; i++) {
    game.objects.plane.geometry.attributes.distortion.array[i] = -Math.random() * 1.1;
  }

  game.objects.plane.geometry.attributes.distortion.needsUpdate = true; // needed to force update the shader with this value

  // properties to update on render
  game.objects.plane.properties.uniforms.time.value += 0.01; 
...
function objectPlane(position, properties){
  ...
  object.properties.attributes.distortion = new Float32Array( object.geometry.attributes.position.count ); // custom attribute

  //object.properties.attributes.rotation = new Float32Array( object.geometry.attributes.position.count ); // custom attribute

  object.geometry.addAttribute( 'distortion', new THREE.BufferAttribute( object.properties.attributes.distortion, 1 ) ); // 1 is 'location' of the attribute inside the shader
